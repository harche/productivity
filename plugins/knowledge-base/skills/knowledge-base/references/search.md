...
```bash
curl -s -H "Authorization: Bearer $ACCESS_TOKEN" \
  "https://access.redhat.com/hydra/rest/search/kcs?q=node+not+ready&rows=10&fq=documentKind:Solution&fl=publishedTitle,view_uri,id,caseCount,lastModifiedDate,boostDetectedVersion" \
  | python3 -c "
import sys, json
data = json.load(sys.stdin)
print(f'Total: {data[\"response\"][\"numFound\"]} solutions')
print(f'{\"ID\":<10} {\"Cases\":<7} {\"Version\":<10} {\"Title\"}')
print('-' * 90)
for doc in data['response']['docs']:
    print(f'{doc[\"id\"]:<10} {doc.get(\"caseCount\",0):<7} {doc.get(\"boostDetectedVersion\",\"N/A\"):<10} {doc.get(\"publishedTitle\",\"\")[:55]}')
"
```
