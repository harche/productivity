...
async function getTranscript(videoId, lang = "en", includeTimestamps = false) {
  return getTranscriptViaDocker(videoId, lang);
}

// Fetch transcript via Docker yt-dlp (bypasses IP blocks and anti-bot detection)
function getTranscriptViaDocker(videoId, lang = "en") {
  const outDir = `/tmp/yt-dlp-transcript-${videoId}`;
  try {
    mkdirSync(outDir, { recursive: true });
    execSync(
      `docker run --rm -v ${outDir}:/out jauderho/yt-dlp ` +
      `--write-auto-sub --sub-lang ${lang} --skip-download --sub-format vtt ` +
      `-o "/out/%(id)s" "https://www.youtube.com/watch?v=${videoId}"`,
      { timeout: 120000, stdio: ["pipe", "pipe", "pipe"] }
    );

    // Find the VTT file
    const files = readdirSync(outDir).filter((f) => f.endsWith(".vtt"));
    if (files.length === 0) throw new Error("No VTT file produced");

    const vtt = readFileSync(`${outDir}/${files[0]}`, "utf-8");

    // Parse VTT: skip headers/timestamps, deduplicate lines
    const seen = new Set();
    const textLines = [];
    ...
    return {
      videoId,
      language: lang,
      languageName: `${lang} (via yt-dlp docker)`,
