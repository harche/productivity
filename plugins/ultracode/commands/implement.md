...
2. Split only when it pays. Most tasks are one writer: you. Use parallel writers when the work decomposes into disjoint files with clear interfaces, and give each one its worktree, its writable files, its read-only dependencies, its acceptance criteria, and the tests to run. Tell writers to come back to you when a contract is ambiguous, when they need a file outside their ownership, or when they learn something another writer needs, rather than guessing.
