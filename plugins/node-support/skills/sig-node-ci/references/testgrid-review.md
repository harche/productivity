...
Weekly SIG Node CI health review: scan the SIG Node TestGrid dashboards for FAILING and FLAKY jobs, determine for each job whether it is already tracked by a GitHub issue in `kubernetes/kubernetes` (or `kubernetes/test-infra` when the tab routes there), and only for UNTRACKED jobs decide whether a new issue is warranted (drafting it for user review).
...
- **FLAKY = failures that never hit 3-in-a-row.** Every FLAKY periodic job still gets a tracking check (TRACKED / REGRESSION? / UNTRACKED) like FAILING jobs. UNTRACKED flakes don't auto-get an issue draft — decide per cluster whether an issue is warranted (frequency, release-blocking impact, age) and ask the user before drafting (they're often better handled via existing flake issues / triage board).
...
1. **Fetch all summaries and bucket the tabs.**
   ...
   ```
   Also collect the FLAKY list (same query with `=="FLAKY"`) for full tracking review — every FLAKY periodic job gets steps 2–3 (details + tracking check), not just a tally name.
...
2. **For each FAILING and each FLAKY periodic job, extract the failure details**: for FAILING use the summary entry's `tests[]`; for FLAKY `tests[]` is empty (no active alert), so sample recent failing cells via the `table` endpoint (`width=10`) or k8s-triage (`https://storage.googleapis.com/k8s-triage/index.html?job=<prow-job>`) to get representative failing test names:
...
3. **Check whether each FAILING / FLAKY job is already tracked.** In order (same for both statuses):
   ...
   - Search `kubernetes/kubernetes` (and `kubernetes/test-infra` when `bug_url` points there) with several strategies, since titles vary:
     ...
     gh search issues --repo kubernetes/kubernetes --state open --label kind/flake --label sig/node --json number,title,url,updatedAt --limit 50  # for FLAKY tabs
     ```
   ...
   - Classify each FAILING / FLAKY job: **TRACKED** (open issue found — link it), **REGRESSION?** (only a recently-closed issue found), or **UNTRACKED** (no open issue — goes to the step-5 issue decision).

4. **Cluster by root cause before drafting — one issue per cause, not per tab.** Failing/flaky tabs usually collapse into far fewer real problems (a live example: 18 FAILING tabs reduced to 5 clusters). Group UNTRACKED FAILING and UNTRACKED FLAKY jobs separately (never mix FAILING and FLAKY in one issue — they take different templates/labels) that share:
...
5. **Decide, then draft an issue only where warranted.** Every UNTRACKED FAILING cluster defaults to draft-worthy — draft it. Every UNTRACKED FLAKY cluster defaults to ask-first — present the tracking table with a draft/no-draft recommendation (weight: release-blocking membership, failure frequency/age, number of affected variants) and draft only the ones the user approves. FAILING drafts use the official template (`kubernetes/kubernetes` `.github/ISSUE_TEMPLATE/failing-test.yaml`, auto-labels `kind/failing-test`); FLAKY drafts use the flake template with `--label kind/flake --label sig/node`. Sections (use these exact headings in the body):
...
6. **Report — and stop.** Present:
   - A summary table of FAILING jobs grouped by cluster: jobs, dashboards, failing tests count, since-when, consecutive fails, tracking status with issue link or "UNTRACKED → draft" / "UNTRACKED → no draft (reason)".
   - A tracking table of FLAKY periodic jobs: job, dashboards, sample failing tests, tracking status with issue link or "UNTRACKED → recommend draft / recommend no draft (reason)".
   - For each UNTRACKED cluster where an issue was decided/approved: the full drafted issue body, followed by a copy-pasteable command the **user** runs:
     ```bash
     ...
     ```
     (For approved FLAKY drafts use `--title "[Flake] <summary>" --label kind/flake --label sig/node`.)
...
- Never draft an issue for an UNTRACKED FLAKY cluster without explicit user approval for that specific draft — present the tracking table and recommendation first. The ≥3-consecutive-failures bar is what auto-justifies `kind/failing-test`; flakes need the step-5 decision.
