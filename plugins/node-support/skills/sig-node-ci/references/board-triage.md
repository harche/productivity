...
- **Closed duplicates go to 'Archive-it', not 'Done'.** 'Done' means the tracked problem was fixed. An issue closed as a duplicate of another card with no fix is noise; archive its card and let the surviving issue's card carry the work. See board-todo-sweep.md *Duplicate Cards* for detection and the `/close` comment.
...
2. **Gather data with `gh`.** Use the GitHub CLI exclusively for board access. Useful commands:
   - `gh project item-list 151 --owner kubernetes --format json --limit 2000 > board.json` to list items. The board has ~1300 cards, so a small limit truncates silently; cache the output to a file and reuse it, because the project GraphQL API rate-limits repeated item-list calls even while `gh api rate_limit` shows budget.
...
5. **Recommend-only by default, but ALWAYS print the commands.** Do NOT execute any board mutation unless the user explicitly asks you to act. However, regardless of whether you are asked to act, your report MUST include a ready-to-run `gh` command for the recommended action on every item, so the user can copy-paste and perform it themselves. This is a hard requirement — never leave the user to construct commands by hand.
   ...
   - **Provide the command matching each item's recommendation, plus the obvious alternatives**, clearly labeled by action:
     - **Accept (keep on board)** → this is TWO actions: (a) apply the Prow triage commands on the issue/PR itself, AND (b) move the board card out of Triage into its destination lane — per the documented process, **issues → 'Issues - To do'** and **PRs → a PR-review lane**. Both (a) and (b) are part of acceptance — do not omit (a). For PRs, (c) below decides the lane and adds the reviewer request to the same comment.
       ...
       (b) Move the card to its destination lane (use the 'Issues - To do' option for issues, or the lane chosen in (c) for PRs):
       `gh project item-edit --id <ITEM_ID> --project-id <PROJECT_ID> --field-id <STATUS_FIELD_ID> --single-select-option-id <DEST_OPTION_ID>`
       (c) **PRs: find the reviewer at triage time.** Cards moved to 'PRs - Needs Reviewer' are never revisited (see the *What the Lane Actually Means* section of [pr-reviewer-finder.md](pr-reviewer-finder.md)), so the acceptance comment is the one moment to request review. For every KEEP PR, run the full pr-reviewer-finder.md workflow on that PR — the gate (step 2), candidate signals (step 3), filter and rank (step 4), and comment draft (step 6). Do not shortcut to OWNERS alone; the point is the ranked, evidence-backed candidate. Then:
         - **Gate failed** (`needs-rebase`, stale, red or pending CI, author hold, draft, author owes a reply) → lane 'PRs Waiting on Author', no `/cc`. Dependency open → note it is BLOCKED and pick 'PRs Waiting on Author'. Superseded → recommend closing and 'Archive-it'.
         - **Already has `lgtm`** → lane 'PRs - Needs Approver', no `/cc` (an approver from OWNERS is the finder's job only if nobody with approve rights is engaged).
         - **Someone is already reviewing** (engaged human in step 3) → lane 'PRs - Needs Reviewer', no `/cc`; name them in the report.
         - **Gate passed, nobody engaged** → lane 'PRs - Needs Reviewer' and fold the `/cc` into the acceptance comment, one comment total. Prow accepts several commands in one body:
           `gh pr comment <NUM> --repo <REPO> --body $'/triage accepted\n/priority <LEVEL>\n/cc @<login>\n@<login> pinging you because <one checkable reason: dir-reviewer tally, OWNERS approver, KEP role>. Happy to find someone else if you are swamped.'`
         Report the candidate with its evidence lines exactly as pr-reviewer-finder.md step 7 describes, so the user can veto or swap the name before anything is posted. Pinging a person is outward-facing; it needs the same explicit approval as any other mutation here.
...
6. **Surface a tally** (e.g., "5 keep, 3 remove, 3 borderline; PRs: 2 needs-reviewer with /cc, 1 waiting on author") and close by flagging the borderline domain calls, the reviewer names awaiting veto, and reminding the user of removal mechanics.
...
- When triaging PRs, the reviewer search in pr-reviewer-finder.md uses REST and Search; check the GraphQL budget before `gh project item-list` and keep Search under 30 calls per minute, as that reference describes. Never drop the search to OWNERS-only to save calls; a bare OWNERS name is not a candidate.
