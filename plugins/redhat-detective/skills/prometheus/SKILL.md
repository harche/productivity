...
4. **`oc whoami -t` may return empty AND exits non-zero.** Kubeconfigs that use client certificate auth have no session token. The non-zero exit code will break `&&` chains, so always use `TOKEN=$(oc whoami -t 2>/dev/null || true)` to absorb the failure. Then check `if [ -z "$TOKEN" ]` and fall back to creating a service account token.
...
if [ -z "$TOKEN" ]; then \
  oc -n openshift-monitoring create sa prometheus-reader 2>/dev/null || true; \
  oc adm policy add-cluster-role-to-user cluster-monitoring-view -z prometheus-reader -n openshift-monitoring 2>/dev/null || true; \
...
Detailed command references — read when you need specifics beyond the examples above:

* **Cluster Access** — [references/cluster-access.md](references/cluster-access.md) — Full discovery, auth, and port-forward setup for OpenShift and Kubernetes
* **Querying** — [references/querying.md](references/querying.md) — instant, range, series, labels, analyze, and PromQL formatting
* **Validation & Testing** — [references/validation.md](references/validation.md) — check config, check rules, check metrics, test rules (unit testing)
* **TSDB Operations** — [references/tsdb.md](references/tsdb.md) — analyze cardinality, list blocks, dump data, create blocks from rules
