...
jira.sh search 'filter = "Node Bugs" AND (priority = Undefined OR "Release Blocker" = Proposed OR assignee in ("aos-node@redhat.com")) AND fixVersion = "4.22.0" AND status not in (Done, CLOSED, Verified) ORDER BY priority DESC' 50

# Blocker+ bugs
jira.sh search 'filter = "Node Bugs" AND ("Release Blocker" = Approved OR priority = Blocker) AND status not in (Done, CLOSED, Verified) ORDER BY priority DESC' 50

# Customer issues older than 100 days
jira.sh search 'filter = "Node Bugs" AND ("Customer Impact" = "Customer Escalated" OR "SFDC Cases Counter" is not EMPTY) AND created <= -100d AND status not in (Done, CLOSED, Verified) ORDER BY created ASC' 50

# My escalations
jira.sh search 'filter = "Node Bugs" AND (project = "Red Hat OpenShift Priority List" OR "Customer Impact" = "Customer Escalated" OR labels in (shift_telco5g)) AND assignee = currentUser() AND status not in (Done, CLOSED, Verified) ORDER BY priority DESC' 50

# CVEs due this week
jira.sh search 'filter = "Node Bugs" AND (labels in (SecurityTracking) OR issuetype in (Vulnerability, Weakness)) AND status not in (Done, CLOSED, Verified, ON_QA) AND due <= 7d ORDER BY due ASC' 50

# My open bugs by version
jira.sh search 'filter = "Node Bugs" AND assignee = currentUser() AND status not in (Done, CLOSED, Verified) ORDER BY fixVersion ASC, priority DESC' 50

# Contract priority without fix version
jira.sh search 'filter = "Node Bugs" AND "Special Handling" in (contract-priority) AND fixVersion is EMPTY AND status not in (Done, CLOSED, Verified, ON_QA) ORDER BY priority DESC' 50

# Green team blockers
jira.sh search 'filter = "Node Bugs" AND ("Release Blocker" = Approved OR priority = Blocker) AND filter = "Node Green Team" AND status not in (Done, CLOSED, Verified) ORDER BY priority DESC' 50

# Component regression bugs
jira.sh search 'filter = "Node Bugs" AND labels = component-regression AND status not in (Done, CLOSED, Verified) ORDER BY priority DESC' 50
```
