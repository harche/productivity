...
Place, modify, cancel, and view orders via the IBKR Web API.
...
```bash
# Place a single order (ALWAYS confirm with user first)
...
```bash
# Confirm/acknowledge order (ALWAYS confirm with user first)
...
```bash
# Modify an existing order (ALWAYS confirm with user first)
...
```bash
# Cancel a specific order (ALWAYS confirm with user first)
