---
name: gmail
description: Search, read, send, and manage Gmail emails, threads, labels, and drafts using the gog CLI. Use when the user asks about email, wants to search/read/send Gmail messages, or manage their inbox.
allowed-tools: Bash(gog:*)
---

# Gmail via gog CLI

Read, search, send, and manage Gmail using the `gog` CLI tool (authenticated via OAuth).
...
- Default: human-friendly colored tables
...
If the user has multiple accounts, specify which one:

```bash
gog gmail search 'newer_than:1d' --account user@gmail.com
```
...
```bash
# Search threads (default, grouped by conversation)
...
- **Always confirm with the user before sending emails, deleting messages, or modifying labels.**
